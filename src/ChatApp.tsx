...
import SetNameImage from './assets/SetName.png';
    ...
    <div className="bg-gray-700 px-10 py-5 text-slate-50 font-poppins font-semibold min-h-screen">
        ...
        <div className="w-2/4 flex flex-col">
          ...
          <div className="relative">
            ...
            <button
              ...
              disabled={isNicknameDisabled}
              className={`text-white absolute right-2.5 bottom-2.5 ${(isNicknameDisabled) ? 'bg-gray-400' : 'bg-[#009b85] hover:bg-[#018673]'} focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2`}
            >
              Establecer
            </button>
          </div>
  ...
  
      {
        (isNicknameDisabled) ? (
              ...
              <div className="py-5 px-10 h-115 overflow-y-scroll">
                {
                  storedMessages?.map(({ message, createdAt, from }) => (
                    ((from === nickname) && (isNicknameDisabled))
                      ? <MyMessage key={createdAt} message={message} createdAt={createdAt} />
                      : <OtherMessage key={createdAt} from={from} message={message} createdAt={createdAt} />
                  ))
                }

                <TextDivider label="Nuevos mensajes" />
          ...
          <div
            className="w-full flex flex-col justify-center items-center mt-16 transition-all"
          >
            <img
              src={SetNameImage}
              alt="Establece un nombre"
              className="w-80"
            />

            <h1
              className="font-poppins text-2xl mt-2"
            >
              Ingresa un nickname para iniciar
            </h1>
          </div>
        )
      }
